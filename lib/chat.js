const { reply, buttons, br, pause } = require('alice-renderer');
...
const chat = (db, key, message) => {
    ...
    switch (chatState.state) {
        case start: {
            ...
            return reply`
                Привет. Назовите, пожалуйста, интересующий вас континент. ${br()} ${br()}
        ...
        case startSettings: {
            ...
            if (chatState.country) {
                ...
                return reply`
                    Что вы хотите выучить? Столицы или Стр+аны? ${br()} ${br()}
